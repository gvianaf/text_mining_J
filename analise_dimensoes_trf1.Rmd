...
correspondencias$termo %>% 
  unlist() %>% 
  enframe() %>% 
  # retiro os termos que não devem fazer parte da análise
  filter(!str_detect(value, "uniao|juizo|supremo|secao|ocasiao|desta|nesse|reconhece|territorial|tributar|apreciacao")) %>% 
  count(value, sort = TRUE) %>% 
  head(50) %>% 
  kable() %>% 
  kable_styling(full_width = FALSE)
